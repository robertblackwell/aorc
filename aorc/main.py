...
def banner():
    def string_pad(candidate, length):
        s = candidate
        while len(s) < length:
            s = s + " "
            if len(s) < length:
    ...
    last_line = "****************************************************"
    aorc_v_line =  string_pad("Aorc version v{}".format(__version__), len(last_line))
    sc_vline =    string_pad("simple_curses version v{}".format(simple_curses_version()), len(last_line))
